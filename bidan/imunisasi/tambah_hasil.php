...
$data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT pi.*, a.nama_anak, a.tanggal_lahir, a.berat_lahir, a.panjang_lahir, u.nama_lengkap as nama_ibu, v.nama_vaksin, j.tanggal 
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    mysqli_query($conn, "INSERT INTO hasil_imunisasi (id_pendaftaran, berat_badan, tinggi_badan, lingkar_kepala, status_gizi, nafsu_makan, catatan_kesehatan, tgl_imunisasi, petugas_nik, created_by) 
        VALUES ('$pendaftaran_id', '$berat', '$tinggi', '$lingkar', '$status_gizi', '$nafsu_makan', '$catatan', CURDATE(), '$petugas_nik', '$created_by')");
...
<div class="max-w-2xl mx-auto fade-in">
    <div class="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div class="bg-green-50 p-4 rounded-lg mb-6 grid grid-cols-2 gap-4">
            <div><p class="text-gray-500 text-sm">Nama Anak</p><p class="font-semibold text-gray-800"><?php echo $data['nama_anak']; ?></p></div>
            <div><p class="text-gray-500 text-sm">Tanggal Lahir</p><p class="font-semibold text-gray-800"><?php echo date('d/m/Y', strtotime($data['tanggal_lahir'])); ?></p></div>
            <div><p class="text-gray-500 text-sm">Nama Ibu</p><p class="font-semibold text-gray-800"><?php echo $data['nama_ibu']; ?></p></div>
            <div><p class="text-gray-500 text-sm">Vaksin</p><p class="font-semibold text-gray-800"><?php echo $data['nama_vaksin']; ?></p></div>
            <div><p class="text-gray-500 text-sm">Tanggal Imunisasi</p><p class="font-semibold text-gray-800"><?php echo date('d/m/Y', strtotime($data['tanggal'])); ?></p></div>
        </div>
        <form method="POST">
            <div class="grid grid-cols-2 gap-4">
                <div><label class="block font-semibold text-gray-700 mb-2">Berat Badan (kg)</label><input type="number" step="0.01" name="berat_badan" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-green-400"></div>
                ...
                <div class="col-span-2"><label class="block font-semibold text-gray-700 mb-2">Catatan Kesehatan</label><textarea name="catatan_kesehatan" rows="3" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-green-400"></textarea></div>
            </div>
            <div class="flex gap-3 mt-6"><button type="submit" class="flex-1 bg-gradient-to-r from-green-600 to-emerald-500 text-white py-2 rounded-xl font-semibold hover:shadow-lg transition">Simpan</button><a href="list_pendaftaran.php" class="flex-1 bg-gray-200 text-gray-700 text-center py-2 rounded-xl font-semibold hover:bg-gray-300 transition">Batal</a></div>
        </form>
    </div>
</div>
